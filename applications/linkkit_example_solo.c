...
static struct dev_properties dev_msg =
{
    .light.name = "MeasuredIlluminance",
    .light.data = 0.0,
};
...
void user_post_property(void)
{
    ...
    light_val = round(1.0 * rand() / RAND_MAX * 10 + 20);

    HAL_Snprintf(property_payload, sizeof(property_payload),
                 "{\"%s\": %d}", dev_msg.light.name, light_val);
